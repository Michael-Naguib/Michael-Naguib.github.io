...
function ArticleCard(props) {
  ...
  return (
    <Link to={article.pLink} key={article.title}>
      <Row
        style={{ color: fontColor, backgroundColor: dark ? '#040719' : 'white', padding: '30px' }}
      >
        <Col md={3}>
...
class ArticlesSearch extends React.Component {
  ...
  render() {
    ...
    if (this.state.searchParam == '') {
      ...
      filteredArticles = this.articles.filter((article) => {
        return Object.keys(article).some((key) =>
          typeof article[key] == typeof ''
            ? article[key].toLowerCase().includes(this.state.searchParam.toLowerCase())
            : false
        )
      })
    }
    ...
    return (
      <div
        id={'ArticlesSearch'}
        style={{ color: fontColor, backgroundColor: dark ? '#040719' : 'white' }}
      >
        ...
        <MathJax.Provider>
          <Container style={{ minHeight: '100vh' }}>
            {filteredArticles.length > 0 ? (
              ...
              <h3 key={'NOARTICLES'}>
                {' '}
                No Articles meet the search criteria "{this.state.searchParam}"
              </h3>
            )}
          </Container>
        </MathJax.Provider>
